<?php
    ...
    if(isset($_POST['submit'])){
        ...
        if(mysqli_num_rows(mysqli_query($conn, "SELECT * FROM users WHERE email='{$email}'")) > 0){
            $msg = "<div class='alert-error'>{$email} - This email has been already exists.</div>";
        } else{
            if($password === $confirm_password){
                $sql = "INSERT INTO users (login, email, password, code) VALUES ('${name}', '${email}', '${password}', '${code}')";
                ...
                if($result){
                    ...
                    try {
                        ...
                        $mail->Body    = 'Here is the verification link <b><a href="http://localhost/loginregisterform/?verification='.$code.'">http://localhost/loginregisterform/?verification='.$code.'</a></b>';
                    ...
                    $msg = "<div class='alert-error'>Something went wrong.</div>";
                }
            } else{
                $msg = "<div class='alert-error'>Password and confirm password do not match.</div>";
            }
        }
    }
?>
